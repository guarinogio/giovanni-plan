...
export function RestTimerProvider({ children }) {
  ...
  const start = (sec) => localStart(sec);
  const pause = () => setPaused(true);
  const resume = () => setPaused(false);
  const add = (sec) => setSecondsLeft((s) => Math.max(0, s + sec));
  const stop = () => { clearInterval(tickRef.current); setActive(false); setPaused(false); setSecondsLeft(0); setDuration(0); };
  const finish = () => { setActive(false); setPaused(false); if (navigator.vibrate) navigator.vibrate([180,120,180]); beep(); };
