...
import RestTimerControl from "./components/timer/RestTimerControl.jsx";
import {
  useLocalStorage,
  roundToStep,
  setDeep,
...
export default function MobilePlanGiovanni() {
  ...
  useEffect(() => {
    const s =
      lastType === "A" ? "B" : lastType === "B" ? "Movilidad" : lastType === "Movilidad" ? "A" : "A";
    setDayType(s);
  ...
  const lastFor = (key) => {
    const f = [...logs].reverse().find((l) => l.exercises[key]);
    return f ? { date: f.date, data: f.exercises[key] } : null;
  };
  ...
  const exportCSV = () => {
    ...
    const rows = [
      ...
      ...lifts.map((l) => {
        const k = l.key.includes("/") ? null : l.key;
        const last = k ? lastFor(k) : null;
        let d = "";
        if (last) {
          const v = last.data;
          if (v?.topWeight !== undefined) d = v.topWeight + "x" + v.topReps + " RPE " + v.topRPE;
          else if (v?.weight !== undefined && v?.reps) d = v.weight + "x" + v.reps.join("/");
        }
        return [l.key, l.name, l.rec, config.units, l.scheme, last ? formatDate(last.date) : "", d];
      })
    ];
    const esc = (x) => {
      const s = String(x ?? "");
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button className="px-3 py-2 rounded-xl bg-neutral-100" onClick={exportCSV}>
              ⬇️ CSV
            </button>
            <button className="px-3 py-2 rounded-xl bg-neutral-100" onClick={resetAllStorage}>
              🧹 Reset
            </button>
            <button className="px-3 py-2 rounded-xl bg-neutral-100" onClick={() => setTab("ajustes")}>
              ⚙️
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 text-sm">
          <TabButton active={tab === "sesion"} onClick={() => setTab("sesion")}>
            Sesión
          </TabButton>
        ...
        {tab === "sesion" && (
          <main className="p-4 space-y-4 pb-36">
            ...
            {dayType === "A" && (
              <div className="space-y-4">
                <StaticListCard
                  ...
                  state={sessionInputs.preA}
                  setState={(p) => handleInput(["preA"], p)}
                />
                <HeavyLiftCard
                  title={"Sentadilla — " + config.variants.squat}
                  hint={
                    "Top 3–5 @ RPE8 · Back-offs 2×5 @ ~90% (~" +
                    roundToStep(weights.squat * 0.9, config.rounding) +
                    " " +
                    config.units +
                    ")"
                  }
                ...
                <AccessoryCard
                  title="Hip Thrust"
                  hint="3×6–10"
                  state={sessionInputs.hipthrust}
                  setState={(p) => handleInput(["hipthrust"], p)}
                  units={config.units}
                  rec={weights.hipthrustAcc}
                  withWeight
                />
                <AccessoryPairCard
                  title="Postura + Brazos"
                  hint="Face Pulls 3×10–15 · Curl Martillo 3×8–12"
                  aLabel="Face Pulls"
                  bLabel="Curl Martillo"
                  state={sessionInputs.posturaBrazosA}
                  setState={(p) => handleInput(["posturaBrazosA"], p)}
                  units={config.units}
                  aRec={weights.facepulls}
                  bRec={weights.curl}
                  aWithWeight
                  bWithWeight
                />
                <AccessoryCard
                  title="Core: Dead Bug"
                  hint="3×8–10"
                  state={sessionInputs.coreA}
                  setState={(p) => handleInput(["coreA"], p)}
                  simple
                  withWeight={false}
                />
                <AccessoryPairCard
                  title="Pies"
                  hint="Gemelos 3×10–15 · Tibialis 3×12–20"
                  aLabel="Gemelos"
                  bLabel="Tibialis"
                ...
                />
                <StaticListCard
                  title="Post-Movilidad"
                  hint="3–5 min suave"
                  items={[
                    { n: "Toe Splay", rec: "2×20–30s" },
                    { n: "Short-Foot", rec: "2×20–30s" }
                  ]}
                  state={sessionInputs.postA}
                  setState={(p) => handleInput(["postA"], p)}
                />
              </div>
            )}
            ...
            {dayType === "B" && (
              <div className="space-y-4">
                <StaticListCard
                  ...
                  state={sessionInputs.preB}
                  setState={(p) => handleInput(["preB"], p)}
                />
                <HeavyLiftCard
                  title={"Peso Muerto — " + config.variants.deadlift}
                  hint={
                    "Top 3–5 @ RPE8 · Back-offs 2×5 @ ~90% (~" +
                    roundToStep(weights.deadlift * 0.9, config.rounding) +
                    " " +
                    config.units +
                    ")"
                  }
                ...
                />
                <AccessoryCard
                  title="Tríceps en cuerda"
                  hint="3×8–12"
                  state={sessionInputs.triceps}
                  setState={(p) => handleInput(["triceps"], p)}
                  units={config.units}
                  rec={weights.triceps}
                  withWeight
                />
                <AccessoryCard
                  title="Core: Pallof / Side Plank"
                  hint="Pallof 3×10/lado o Plank 2–3×20–30s"
                  state={sessionInputs.coreB}
                  setState={(p) => handleInput(["coreB"], p)}
                  simple
                  withWeight={false}
                />
                <AccessoryPairCard
                ...
                <StaticListCard
                  ...
                  state={sessionInputs.postB}
                  setState={(p) => handleInput(["postB"], p)}
                />
              </div>
            )}
            ...
            {dayType === "Movilidad" && (
              <div className="space-y-4">
                <StaticListCard
                  ...
                  state={sessionInputs.morn}
                  setState={(p) => handleInput(["morn"], p)}
                />
                <StaticListCard
                  title="Durante el día (oficina/postura)"
                  hint="micro-pauses"
                  items={[
                  ...
                  state={sessionInputs.day}
                  setState={(p) => handleInput(["day"], p)}
                />
              </div>
            )}
            ...
            <button
              ...
              className="fixed bottom-6 left-4 right-4 py-4 rounded-2xl text-center font-semibold bg-indigo-600 text-white shadow-md"
            >
              Guardar Sesión
            </button>
          </main>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 items-end">
                <div>
                  ...
                  <select
                    className="mt-1 w-full px-3 py-2 rounded-xl bg-neutral-100"
                    value={config.units}
...
function TabButton({ active, onClick, children }) {
  return (
    <button
      onClick={onClick}
      className={
        "py-2 rounded-xl font-medium " + (active ? "bg-neutral-900 text-white" : "bg-neutral-100")
      }
    >
      {children}
    </button>
